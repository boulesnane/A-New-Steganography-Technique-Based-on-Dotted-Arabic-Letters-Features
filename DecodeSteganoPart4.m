...
%-------------------------------------------------------------------------
function [covertext, originaltext] = DecodeSteganoPart4(steganotext,mappingtable)
clc
bits = 6;
kashidadec = 1600;
    ...
    if value == 1
        ...
        if findit == false
            if  steganotext(j+stp+1)==kashidadec
                if ismember(char(steganotext(j)), tp) || ismember(char(steganotext(j)), bp)
                    stopcount=stopcount+1;
                    
                    steganotext(j+stp+1)=[];
                    originaltextbits=[originaltextbits 1];
                    
                elseif ismember(char(steganotext(j)), np)
                    stopcount=0;
                    steganotext(j+stp+1)=[];
