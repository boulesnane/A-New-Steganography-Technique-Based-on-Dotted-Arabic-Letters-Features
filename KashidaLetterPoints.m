...
%-------------------------------------------------------------------------
function [steganotext1,mappingtable, Stegobitsused,Secretbits] = KashidaLetterPoints(covertext, secrettext)
clc
        ...
        if value == 1
            ...
            if findit == false
                ...
                if  str2num(secrtbits(i))==1
                    if ismember(char(steganotext1(j)), tp) || ismember(char(steganotext1(j)), bp)
                ...
                elseif str2num(secrtbits(i))==0
                    if ismember(char(steganotext1(j)), np)
                        Stegobitsused = Stegobitsused + 1;
                        histoBits =[histoBits 1];
                        steganotext1 =[steganotext1(1:j+stp)  kashidadec steganotext1(j+stp+1:length(steganotext1))];
                        j= j+1;
